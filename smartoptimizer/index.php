...
{
	error_log("SmartOptimizer Error: ".str_replace("\n", "\\n", addslashes($msg)), 1, "marco@naturalborncamper.com");
	global $settings;
	if (!$settings['debug']) {
...
function debugContinue($msg)
{
	global $settings;
	if (!$settings['debug']) {
		return;
	}
	error_log("SmartOptimizer Error: ".str_replace("\n", "\\n", addslashes($msg)), 3, "_error_log.log");
//	error_log("SmartOptimizer Error: ".str_replace("\n", "\\n", addslashes($msg)), 1, "marco@montrealzombiewalk.com");
}
...
function filesmtime()
{
	...
	foreach ($files as $file) {
		if (!file_exists($file)) continue;//debugExit("File not found ($file).");
...
//error_log("_SERVER: ".print_r($_SERVER, true), 1, "marco@naturalborncamper.com");
//error_log("query: ".print_r($query, true), 1, "marco@naturalborncamper.com");

// SCRAPPE tout jusqu'au premier slash
//if (preg_match('/^\/?(.+\/)?(.+)$/', $query, $matchResult))

// comme d'hab (marche po
//if (preg_match('/^\/?(.+\/)?(.+\..+)$/', $query, $matchResult))

// doit être comme ça: src=@fichier1.js,fichier2.js,fichier3.js ou inc/js/@fichier1.js,fichier2.js
// '@' marque la fin du chemin où sont les fichiers et le début de la liste des fichiers
if (preg_match('/^\/?(.+)?@(.+)$/', $query, $matchResult))
{
...
//error_log("fileNames: ".print_r($fileNames, true), 1, "marco@naturalborncamper.com");
//error_log("fileDir: ".print_r($fileDir, true), 1, "marco@naturalborncamper.com");
...
//error_log("fileNames: ".print_r($fileNames, true), 1, "marco@naturalborncamper.com");
//error_log("filesConcat: ".print_r($files, true), 1, "marco@naturalborncamper.com");
...
if (!$settings['clientCache'] || !$settings['clientCacheCheck'] || !isset($_SERVER['HTTP_IF_MODIFIED_SINCE']) || $_SERVER['HTTP_IF_MODIFIED_SINCE'] != $mtimestr)
{
	...
	if ($generateContent)
	{
		...
		$content = array();
//error_log("files: ".print_r($files, true), 1, "marco@naturalborncamper.com");
		foreach ($files as $file) (($content[] = @file_get_contents($file)) !== false) || debugContinue("File not found ($file).");
		$content = implode("\n", $content);
